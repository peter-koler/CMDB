#!/usr/bin/env python3
"""
同步 HertzBeat 服务器模板到本项目：
1) 复制 define/app-*.yml 到 backend/templates/
2) 注入服务器默认 alerts
3) Upsert 到 monitor_templates 表，确保模板菜单可见
"""
...
from scripts.apply_server_default_alerts import POLICIES, _render_alerts, _upsert_alerts
...
def _parse_template_meta(content: str, fallback_app: str) -> tuple[str, str, str]:
    ...
    category = (category_match.group(1).strip().lower() if category_match else "server") or "server"
...
def main() -> int:
    ...
    with flask_app.app_context():
        ...
        template_service.save_category("服务器", "server", "hdd")
