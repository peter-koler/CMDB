#!/usr/bin/env python3
"""
同步 HertzBeat 网络交换机模板到本项目：
1) 复制 define/app-*.yml 到 backend/templates/
2) 保持 category=network
3) 注入默认 alerts
4) Upsert 到 monitor_templates，并确保分类 network 存在
"""
...
from scripts.apply_network_default_alerts import POLICIES, _render_alerts, _upsert_alerts
...
def _parse_template_meta(content: str, fallback_app: str) -> tuple[str, str, str]:
    ...
    category = (category_match.group(1).strip().lower() if category_match else "network") or "network"
...
def main() -> int:
    ...
    with flask_app.app_context():
        ...
        template_service.save_category("网络设备", "network", "global")
    ...
    print(f"[sync-network] copied files: {len(copied)}")
    print(f"[sync-network] upserted templates: {len(upserted)} -> {', '.join(upserted)}")
    if missing:
        print("[sync-network] missing source files:")
        for item in missing:
            print(f"  - {item}")
    return 0
