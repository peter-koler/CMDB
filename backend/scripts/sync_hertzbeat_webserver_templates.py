#!/usr/bin/env python3
"""
同步 HertzBeat WebServer 模板到本项目：
1) 复制 define/app-*.yml 到 backend/templates/
2) 优先应用 template_overrides（Jolokia/HTTP 可落地模板）
3) 注入 WebServer 默认 alerts
...
from scripts.apply_webserver_default_alerts import POLICIES, _render_alerts, _upsert_alerts
...
def _parse_template_meta(content: str, fallback_app: str) -> tuple[str, str, str]:
    ...
    category = (category_match.group(1).strip().lower() if category_match else "webserver") or "webserver"
...
def main() -> int:
    ...
    with flask_app.app_context():
        ...
        # 确保分类树可见
        template_service.save_category("Web服务器", "webserver", "deployment-unit")
    ...
    print(f"[sync-webserver] copied files: {len(copied)}")
    print(f"[sync-webserver] upserted templates: {len(upserted)} -> {', '.join(upserted)}")
    if missing:
        print("[sync-webserver] missing source files:")
        for item in missing:
            print(f"  - {item}")
    return 0
