...
class CmdbModel(db.Model):
    ...
    category_id = db.Column(
        db.Integer, db.ForeignKey("model_categories.id"), nullable=False
    )
    model_type_id = db.Column(
        db.Integer, db.ForeignKey("model_types.id"), nullable=True
    )
...
@event.listens_for(CmdbModel, "before_delete")
def invalidate_triggers_on_model_delete(mapper, connection, target):
    from app.models.cmdb_relation import RelationTrigger

    connection.execute(
        RelationTrigger.__table__.update()
        .where(
            (RelationTrigger.source_model_id == target.id)
            | (RelationTrigger.target_model_id == target.id)
        )
