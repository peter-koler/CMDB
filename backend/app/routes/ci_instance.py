...
def delete_instance(id):
    ...
    # 记录操作日志
    log_operation(int(identity), claims.get('username'), 'DELETE', 'ci_instance', id, 
...
def _log_ci_history(ci_id, operation, attribute_name, old_value, new_value, operator_id, operator_name):
    """记录CI变更历史"""
    history = CiHistory(
        ci_id=ci_id,
        operation=operation,
...
def batch_update():
    ...
    log_operation(int(identity), claims.get('username'), 'UPDATE', 'ci_instance', None, 
...
def batch_delete():
    ...
    log_operation(int(identity), claims.get('username'), 'DELETE', 'ci_instance', None, 
...
def export_instances():
    ...
    fields = ModelField.query.filter_by(model_id=model_id, is_active=True).order_by(ModelField.sort_order).all()
    
    output = io.StringIO()
    headers = ['CI编码', 'CI名称', '部门', '创建人', '创建时间']
    field_names = ['code', 'name', 'department_name', 'created_by_username', 'created_at']
...
def import_instances():
    ...
    try:
        ...
        log_operation(int(identity), claims.get('username'), 'CREATE', 'ci_instance', None, 
                      f'批量导入 {success_count} 个CI')
        
        return jsonify({
            'code': 200,
            'message': f'导入完成，成功{success_count}条' + (f'，失败{len(error_messages)}条' if error_messages else ''),
            'data': {
