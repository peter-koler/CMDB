...
## 11. 2026-02-15 持续增量更新（字典管理与模型设计增强）

> 说明：本章为第 10 章之后的持续实现记录，聚焦“字典管理、模型设计器、CI 详情资产展示、模型类型字典化”等增强项。若与前文冲突，以本章口径为准。

### 11.1 新增字典管理能力（配置管理）
- 新增菜单：`配置管理 -> 字典管理`。
- 页面结构调整为左右布局：
  - 左侧：字典类型列表（列表结构），顶部使用 `+/-` 快捷新增/删除，附编辑与刷新按钮。
  - 右侧：当前字典类型对应的字典项数据。
- 支持能力：
  - 字典类型增删改（编码、名称、状态、排序、描述）
  - 字典项增删改（编码、显示名、层级 parent_id、状态、排序）
  - 树形字典结构（层级字典）
- 审计范围：仅记录字典类型/字典项增删改操作；查询不入审计日志。

### 11.2 后端字典模型与接口（已落地）
- 新增模型：
  - `CmdbDictType`（字典类型）
  - `CmdbDictItem`（字典项，支持 parent_id 树结构）
- 新增接口：
  - `GET /api/v1/cmdb/dict/types`
  - `POST /api/v1/cmdb/dict/types`
  - `PUT /api/v1/cmdb/dict/types/:id`
  - `DELETE /api/v1/cmdb/dict/types/:id`
  - `GET /api/v1/cmdb/dict/types/:id/items`
  - `POST /api/v1/cmdb/dict/types/:id/items`
  - `PUT /api/v1/cmdb/dict/items/:id`
  - `DELETE /api/v1/cmdb/dict/items/:id`
  - `GET /api/v1/cmdb/dict/items/by-type-code/:type_code`
- 约束与兼容：
  - 同级编码唯一
  - 防止节点移动到自身子节点下
  - 删除允许（保留历史值展示能力由业务展示层处理）

### 11.3 模型设计器增强（ModelDesigner）

#### 11.3.1 控件列表布局优化
- 控件列表从“单列”升级为“默认 3 列网格”。
- 响应式缩放：
  - 大屏 3 列
  - 中屏 2 列
  - 窄屏 1 列
...
#### 11.3.2 选项控件支持字典数据源
- 下拉/单选/多选新增“字典”选项类型（替代原动态占位选项）。
- 可选择字典类型编码，自动加载字典项并预览。
- 保存时将字典选项标准化写入 `options`，供 CI 动态表单直接渲染。

#### 11.3.3 人员控件支持系统用户
- 人员控件新增“候选人员”配置（从系统用户选择）。
- 支持单选/多选模式。
- 保存时将候选人员选项标准化写入配置，CI 页面可直接使用。

#### 11.3.4 属性分组控件增强
- 分组支持折叠/展开，折叠状态持久化（`collapsed`）。
- 新增分组编码 `groupCode`（非必填，模型内唯一校验）。
- 分组标题支持头部内联编辑（回车/失焦保存）。
- 字段支持“移动到分组”快捷菜单 + 属性面板快捷切组（与拖拽并存）。
- 无分组字段统一按“基础属性”逻辑处理。
- 修复分组内字段占用列数（span）不生效问题：
  - 分组内字段渲染改为 `row/col(span)`，与默认属性一致。
- 修复分组高度被裁剪问题：
  - 分组容器改为内容驱动高度，随字段数量自动增长。

#### 11.3.5 表格控件修复与可用化
- 修复表格控件“配置不生效”问题（统一以 `columns[]` 作为单一事实源）。
- 新增列配置编辑：
  - 列名（title）
  - 字段编码（dataIndex）
  - 列宽（width）
  - 增删列
- 画布和预览均按列配置渲染表格。
- 兼容历史结构：若旧数据为数字列数，自动转换为列数组。
- 新增保存校验：
  - 至少一列
  - `dataIndex` 不能为空
  - 同表内 `dataIndex` 不可重复

### 11.4 CI 新增/编辑（动态表单）增强
- `CiInstanceModal` 增强对 `form_config` 的解析：
  - 支持分组字段解析
  - 支持字典选项字段
  - 支持人员控件（系统用户候选）字段
- 动态字段渲染能力补齐：
  - 人员控件在新增/编辑时可真实选择（非占位）。
- CI 新增/编辑页面支持按分组展示属性：
  - 分组字段按组渲染
  - 无分组字段归入“基础属性”

### 11.5 CI 详情页资产展示优化（属性信息 Tab）
- 图片控件：
  - 改为缩略图展示（支持单个/多个）
  - 点击缩略图新窗口打开原图
- 附件控件：
  - 展示“文件名 + 文件大小”
  - 点击可下载
  - 兼容值格式：字符串 URL / 对象 / 数组 / JSON 字符串
- 文件大小：
  - 优先使用值内 size
  - 缺失时通过 `HEAD content-length` 自动探测并缓存

### 11.6 模型管理“模型类型”字典化
- 模型管理页“新增/编辑模型”中的“模型类型”改为读取字典 `mode_type`。
- 提交时写入 `config.model_type_code`（不再依赖旧 `model_type_id`）。
- 列表展示优先显示字典映射名称；无映射时回退旧字段显示（兼容历史数据）。

### 11.7 文件访问 404 修复（后端）
- 修复 CI 图片/附件 URL 可访问性问题：
  - 上传目录从相对路径改为绝对路径（固定到 `backend/uploads`）
  - 下载接口增加历史目录兜底查找（兼容旧文件目录）
- 目标：避免因服务启动目录变化导致 `/api/v1/cmdb/instances/files/...` 404。

### 11.8 本章验收补充
- [ ] 字典管理页面为左右结构，左侧列表 + `+/-` 快捷操作，右侧展示字典项数据。
- [ ] 字典类型与字典项的增删改可用，查询不进入审计日志。
- [ ] 模型设计器控件列表默认 3 列，并随窗口缩放自适应。
- [ ] 下拉/单选/多选可绑定字典并在 CI 动态表单中生效。
- [ ] 人员控件可选择系统用户并在 CI 动态表单中生效。
- [ ] 属性分组支持折叠、分组编码、内联改名、快捷切组，且分组内 span 生效。
- [ ] 属性分组容器高度随字段数量自适应，不出现内容裁剪。
- [ ] 表格控件可配置列定义并在画布/预览中生效，保存校验正确拦截异常列配置。
- [ ] CI 详情页图片以缩略图展示并可点击新窗口打开。
- [ ] CI 详情页附件展示名称与大小，点击可下载且不再报错。
- [ ] 图片/附件文件 URL 在后端路由下可稳定访问（不因启动目录变化而 404）。

### 11.9 评审口径（冲突消解）
- 模型类型评审口径：以字典 `mode_type` 为准；旧 `model_type_id` 仅作历史兼容。
- 分组字段布局评审口径：分组内与分组外字段 span 规则一致。
- 表格控件评审口径：以 `columns[]` 配置驱动渲染，不再使用 `columnCount` 旧口径。
- 资产展示评审口径：图片按缩略图+外链查看，附件按名称/大小/下载展示。
