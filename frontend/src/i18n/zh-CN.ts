export default {
  common: {
    confirm: '确认',
    cancel: '取消',
    save: '保存',
    delete: '删除',
    edit: '编辑',
    add: '新增',
    search: '搜索',
    reset: '重置',
    back: '返回',
    submit: '提交',
    success: '操作成功',
    error: '操作失败',
    warning: '警告',
    info: '提示',
    logout: '退出登录',
    darkMode: '深色模式',
    lightMode: '浅色模式',
    action: '操作',
    confirmDelete: '确认删除',
    all: '全部',
    saveSuccess: '保存成功',
    saveFailed: '保存失败',
    view: '查看',
    status: '状态',
    active: '启用',
    inactive: '禁用',
    yes: '是',
    no: '否',
    total: '共 {total} 条'
  },
  login: {
    title: 'IT运维平台',
    username: '用户名',
    password: '密码',
    loginBtn: '登录',
    usernamePlaceholder: '请输入用户名',
    passwordPlaceholder: '请输入密码',
    rememberMe: '记住密码'
  },
  menu: {
    dashboard: '首页',
    cmdb: 'CMDB',
    instance: '配置仓库',
    search: '全文搜索',
    history: '变更历史',
    topology: '拓扑视图',
    topologyTemplate: '拓扑模板',
    topologyManage: '拓扑图',
    config: '配置管理',
    model: '模型管理',
    dictionary: '字典管理',
    relationType: '关系类型',
    relationTrigger: '关系触发器',
    system: '系统管理',
    user: '用户管理',
    department: '部门管理',
    role: '角色管理',
    systemConfig: '系统配置',
    log: '日志审计',
    notification: '通知管理',
    sendNotification: '发送通知',
    monitoring: '监控管理',
    monitoringTemplate: '监控模板',
    monitoringList: '监控列表',
    monitoringBulletin: '监控简报',
    monitoringAlert: '告警管理',
    monitoringAlertCenter: '告警中心',
    monitoringAlertCurrent: '当前告警',
    monitoringAlertHistory: '告警历史',
    monitoringAlertRule: '告警规则',
    monitoringAlertIntegration: '告警集成',
    monitoringAlertGroup: '告警分组',
    monitoringAlertInhibit: '告警抑制',
    monitoringAlertSilence: '告警静默',
    monitoringAlertNotice: '通知配置',
    monitoringDashboard: '监控展示',
    monitoringCollector: '采集器管理',
    monitoringLabels: '标签管理',
    monitoringStatus: '状态页'
  },
  user: {
    title: '用户管理',
    username: '用户名',
    email: '邮箱',
    phone: '手机号',
    department: '部门',
    role: '角色',
    status: '状态',
    createdAt: '创建时间',
    actions: '操作',
    addUser: '新增用户',
    editUser: '编辑用户',
    deleteUser: '删除用户',
    resetPassword: '重置密码',
    unlock: '解锁',
    roleAdmin: '管理员',
    roleUser: '普通用户',
    statusActive: '正常',
    statusLocked: '锁定',
    statusDisabled: '禁用',
    profile: '个人中心',
    admin: '管理员',
    user: '普通用户'
  },
  profile: {
    basicInfo: '基本信息',
    changeAvatar: '更换头像',
    emailPlaceholder: '请输入邮箱地址',
    phonePlaceholder: '请输入手机号码',
    invalidEmail: '请输入有效的邮箱地址',
    invalidPhone: '请输入有效的手机号码',
    changePassword: '修改密码',
    oldPassword: '旧密码',
    newPassword: '新密码',
    confirmPassword: '确认密码',
    oldPasswordRequired: '请输入旧密码',
    newPasswordRequired: '请输入新密码',
    confirmPasswordRequired: '请确认新密码',
    passwordMismatch: '两次输入的密码不一致',
    passwordChanged: '密码修改成功',
    passwordChangeFailed: '密码修改失败',
    avatarTooLarge: '头像文件不能超过2MB',
    avatarUpdated: '头像更新成功',
    avatarUploadFailed: '头像上传失败'
  },
  config: {
    title: '系统配置',
    token: 'Token配置',
    password: '密码策略',
    login: '登录安全',
    log: '日志配置',
    accessTokenExpire: 'Access Token有效期(分钟)',
    refreshTokenExpire: 'Refresh Token有效期(分钟)',
    idleLogoutMinutes: '无操作自动登出时间(分钟)',
    passwordMinLength: '密码最小长度',
    passwordForceChangeDays: '强制修改密码周期(天)',
    passwordHistoryCount: '密码历史检查次数',
    maxLoginFailures: '最大登录失败次数',
    lockDurationHours: '账户锁定时长(小时)',
    logRetentionDays: '日志保留天数',
    requireUppercase: '密码需要大写字母',
    requireLowercase: '密码需要小写字母',
    requireDigit: '密码需要数字',
    requireSpecial: '密码需要特殊字符'
  },
  log: {
    title: '日志审计',
    username: '操作用户',
    operationType: '操作类型',
    operationObject: '操作对象',
    operationDesc: '操作描述',
    ipAddress: 'IP地址',
    status: '状态',
    createdAt: '操作时间'
  },
  notifications: {
    title: '通知中心',
    send: '发送通知',
    sendNotification: '发送通知',
    markRead: '标记已读',
    markUnread: '标记未读',
    markReadSuccess: '标记已读成功',
    markUnreadSuccess: '标记未读成功',
    batchReadSuccess: '批量标记已读成功',
    sendSuccess: '发送成功',
    sendFailed: '发送失败',
    type: '通知类型',
    status: '状态',
    read: '已读',
    unread: '未读',
    sender: '发送者',
    time: '时间',
    content: '内容',
    notificationTitle: '标题',
    search: '搜索',
    searchPlaceholder: '搜索通知标题或内容',
    selectUsers: '选择用户',
    selectUsersPlaceholder: '请选择接收用户',
    selectDepartment: '选择部门',
    selectDepartmentPlaceholder: '请选择接收部门',
    selectType: '请选择通知类型',
    recipientType: '接收者类型',
    specificUsers: '指定用户',
    department: '部门',
    broadcast: '全员广播',
    preview: '预览',
    previewTitle: '通知标题',
    previewContent: '通知内容预览...',
    titlePlaceholder: '请输入通知标题',
    contentPlaceholder: '请输入通知内容，支持Markdown格式',
    markdownSupport: '支持Markdown格式：**粗体**、*斜体*、`代码`等',
    deleteConfirm: '确定要删除选中的 {count} 条通知吗？',
    deleteSuccess: '成功删除 {count} 条通知'
  },
  validations: {
    required: '此项为必填项',
    maxLength: '最多输入{max}个字符'
  },
  template: {
    title: '监控模板管理',
    app: '应用标识',
    appPlaceholder: '请输入应用标识，如：mysql',
    category: '分类',
    categoryPlaceholder: '请选择分类',
    categoryDb: '数据库',
    categoryService: '服务',
    categoryOs: '操作系统',
    categoryMiddleware: '中间件',
    categoryCustom: '自定义',
    name: '模板名称',
    metrics: '指标数量',
    protocol: '协议',
    add: '新增模板',
    addTitle: '新增监控模板',
    editTitle: '编辑监控模板',
    viewTitle: '查看模板详情',
    confirmDelete: '确定要删除此模板吗？',
    deleteSuccess: '删除成功',
    deleteFailed: '删除失败',
    addSuccess: '新增成功',
    editSuccess: '编辑成功',
    saveFailed: '保存失败',
    statusChangeSuccess: '状态变更成功',
    statusChangeFailed: '状态变更失败',
    form: {
      app: '应用标识',
      appPlaceholder: '请输入应用标识',
      appRequired: '请输入应用标识',
      category: '分类',
      categoryPlaceholder: '请选择分类',
      categoryRequired: '请选择分类',
      nameZh: '中文名称',
      nameZhPlaceholder: '请输入中文名称',
      nameZhRequired: '请输入中文名称',
      nameEn: '英文名称',
      nameEnPlaceholder: '请输入英文名称',
      protocol: '协议',
      protocolPlaceholder: '请选择协议',
      protocolRequired: '请选择协议',
      paramsConfig: '参数配置',
      params: '参数列表',
      addParam: '添加参数',
      paramField: '字段名',
      paramNameZh: '显示名称',
      paramType: '类型',
      required: '必填',
      metricsConfig: '指标配置',
      metrics: '指标组',
      addMetric: '添加指标组',
      metric: '指标组',
      metricName: '指标组名称',
      metricPriority: '优先级',
      metricFields: '指标字段',
      fieldName: '字段名',
      fieldType: '类型',
      fieldUnit: '单位',
      fieldNameZh: '显示名称',
      addField: '添加字段',
      deleteMetric: '删除指标组'
    },
    view: {
      basicInfo: '基本信息',
      params: '参数配置',
      metrics: '指标配置',
      priority: '优先级',
      paramField: '字段名',
      paramName: '显示名称',
      paramType: '类型',
      required: '必填',
      fieldName: '字段名',
      fieldType: '类型',
      fieldUnit: '单位',
      fieldDisplayName: '显示名称'
    },
    categories: '监控分类',
    categoryCloud: '云服务',
    categoryNetwork: '网络设备',
    duplicate: '复制',
    duplicateSuccess: '复制成功',
    preview: '预览',
    saveAndApply: '保存并应用',
    saveSuccess: '保存成功',
    addNewType: '新增监控类型',
    selectTemplate: '请选择监控模板',
    deleteMonitorType: '删除监控类型',
    addCategory: '新增分类',
    addSubCategory: '新增子分类',
    rename: '重命名',
    renameCategory: '重命名分类',
    categoryName: '分类名称',
    categoryNamePlaceholder: '请输入分类名称',
    categoryNameRequired: '请输入分类名称',
    categoryCode: '分类编码(category)',
    categoryCodePlaceholder: '请输入分类编码，例如 server、webserver、network_device',
    categoryCodeRequired: '请输入分类编码',
    categoryCodeInvalid: '分类编码仅支持小写字母、数字、下划线、中划线，且必须以字母或数字开头',
    categoryCodeDuplicate: '分类编码已存在，请更换',
    confirmDeleteCategory: '确认删除分类',
    confirmDeleteCategoryContent: '删除分类将同时删除该分类下的所有子分类，是否继续？',
    previewDetail: {
      basicInfo: '基本信息',
      params: '参数配置',
      metrics: '指标配置',
      priority: '优先级',
      paramField: '字段名',
      paramName: '显示名称',
      paramType: '类型',
      required: '必填',
      defaultValue: '默认值',
      protocol: '协议',
      fields: '指标字段',
      fieldName: '字段名',
      fieldType: '类型',
      fieldUnit: '单位',
      fieldLabel: '标签',
      noMetrics: '暂无指标配置'
    }
  }
}
